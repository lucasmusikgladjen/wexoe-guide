...
| Wexoe Landing Page | `[wexoe_landing]` | Modulär landing page med hero, sidebar-varianter, tabs och kontaktperson | 1.0.0 |
...
**Shortcode:** `[wexoe_landing]`

**Beskrivning:** Renderar en komplett, modulär landing page driven av Airtable-data. Stöder hero med diagonal bildklippning, content-sektion med 4 olika sidebar-typer, tab-sektion med 7 olika content-typer, och kontaktperson med citat. Designad för kampanjsidor, produktlanseringar, event och lead generation.

### Användning

```
[wexoe_landing slug="optix"]
```

### Debug-läge

```
[wexoe_landing slug="optix" debug="true" nocache="true"]
```
...
|-----------|---------|-------------|
| `slug` | *(obligatorisk)* | Matchar Airtable-fältet `Slug`. Ex: `optix`, `ftto`, `ftto-webinar` |
| `nocache` | `false` | Skippa transient-cache (5 min). Användbart vid test. |
| `debug` | `false` | Visa debug-info (raw Airtable-data, tab-mappning). |
| `remove_padding` | `false` | Nollställ Enfold-sektionens padding runt shortcoden. |

### Airtable-tabeller

**Base ID:** `appXoUcK68dQwASjF`

| Tabell | Table ID | Beskrivning |
|--------|----------|-------------|
| Landing Pages | `tbl8KDqGq0Ray1uqS` | Huvudtabell — en rad per sida (37 fält) |
| LP Tabs | `tblvecOh3rAGmw3mw` | Tabs — linked till Landing Pages (24 fält) |
| LP Downloads | `tblbLM827DzjWGjCR` | Nedladdningar — linked till LP Tabs (8 fält) |

### Sektioner (fast ordning)

Alla sektioner renderas villkorligt baserat på checkboxfält. Om data saknas eller checkbox är avbockad visas sektionen inte.

#### 1. Hero

Mörkblå sektion med diagonal bildklippning (clip-path polygon 25%). Bilden täcker höger 55% med gradient-fade. Mobil: bilden visas ovanför texten med gradient-fade nedåt, CTA-knappar blir full-width stacked.

| Fält | Typ | Beskrivning |
|------|-----|-------------|
| `H1` | singleLineText | Sidans huvudrubrik |
| `Hero Description` | multilineText | Brödtext under H1 |
| `Hero Image` | url | Bild-URL (WP Media eller extern) |
...
Tvåkolumns-layout: content (55%) + modulär sidebar (40%).

**Content-fält:**

| Fält | Typ | Beskrivning |
|------|-----|-------------|
| `Content H2` | singleLineText | Rubrik |
| `Content Text` | multilineText | Brödtext (stöder markdown: **bold**, *italic*, [link](url)) |
| `Content Benefits` | multilineText | En benefit per rad → gröna checkmarks |
| `Show Content` | checkbox | Visa/dölj |
| `Show Sidebar` | checkbox | Visa/dölj |

**Sidebar-typer** (väljs via single select `Sidebar Type`):

##### `case` — Kundcase

| Fält | Beskrivning |
|------|-------------|
| `Case Title` | Rubrik ("KUNDCASE"-badge + titel) |
| `Case Description` | Kort beskrivning |
| `Case Image` | Valfri bild (url) |
| `Case Outcomes` | En per rad → gröna checkmarks |
| `Case CTA Text` | Länktext |
| `Case CTA URL` | Länk-URL |

##### `calculator` — Interaktivt element

| Fält | Beskrivning |
|------|-------------|
| `Calc Title` | Rubrik ovanför kalkylatorn |
| `Calc HTML` | Komplett HTML/CSS/JS som renderas som innerHTML i en canvas-div |

##### `event` — Event/webbinarium

| Fält | Beskrivning |
|------|-------------|
| `Event Type` | Fritext (t.ex. "Webbinarium", "Teknikorientering") |
| `Event Title` | Huvudrubrik |
| `Event Description` | Kort text |
| `Event Date` | Fritext (t.ex. "18 september 2026, kl 10:00–11:00") |
| `Event Location` | Fritext (t.ex. "Online (Microsoft Teams)") |
| `Event Webhook` | Make.com webhook-URL — email skickas hit vid anmälan |

Formulär: email → "Anmäl mig"-knapp (orange).

##### `leadmagnet` — Lead magnet / nedladdning

| Fält | Beskrivning |
|------|-------------|
| `Magnet Title` | Assetets rubrik |
| `Magnet Format` | Formatbeskrivning (t.ex. "PDF · 16 sidor · 2026") |
| `Magnet Description` | Pitch-text |
| `Magnet File URL` | URL till fil/resurs — öppnas efter submit |
| `Magnet Webhook` | Make.com webhook-URL — email skickas hit |

Formulär: email + consent-checkbox → "Ladda ner"-knapp (orange). "GRATIS RESURS"-badge i mörkblått.

#### 3. Tabs

Segmented control pill-bar som sitter mellan vit och mörkblå bakgrund. Tabs hämtas från tabell `LP Tabs` via linked records.

**Generella tab-fält (LP Tabs):**

| Fält | Typ | Beskrivning |
|------|-----|-------------|
| `Name` | singleLineText | Tab-knappens text |
| `Tab Type` | singleSelect | Typ av content (se nedan) |
| `Order` | number | Sorteringsordning |
| `Visa` | checkbox | Visa/dölj tab |
| `Landing Page` | multipleRecordLinks | Koppling till Landing Pages |

`Show Tabs` (checkbox på Landing Page) måste vara true.

##### Tab-typer

**`textimage`** — Text + bild (halv/halv)

| Fält | Beskrivning |
|------|-------------|
| `TI H2` | Rubrik |
| `TI Text` | Brödtext |
| `TI Benefits` | En per rad → vita checkmarks |
| `TI Image` | Bild-URL (url) |
| `TI Inverted` | Checkbox — bild vänster, text höger |

**`fullmedia`** — Helbild eller YouTube-video

| Fält | Beskrivning |
|------|-------------|
| `FM URL` | Bild-URL eller YouTube-URL (auto-detect) |

**`faq`** — Accordion FAQ

| Fält | Beskrivning |
|------|-------------|
| `FAQ Items` | Multiline. Format: `Q: Fråga\nA: Svar\n\nQ: Fråga 2\nA: Svar 2` |

**`calameo`** — Upp till 3 Calameo-iframes

| Fält | Beskrivning |
|------|-------------|
| `Calameo 1 Title` + `Calameo 1 Src` | Rubrik + iframe-URL |
| `Calameo 2 Title` + `Calameo 2 Src` | (valfri) |
| `Calameo 3 Title` + `Calameo 3 Src` | (valfri) |

**`downloads`** — Nedladdningskort (grid)

Hämtar records från tabell `LP Downloads` via linked record `LP Downloads` på tabben.

| Fält (LP Downloads) | Beskrivning |
|---------------------|-------------|
| `Name` | Kortets rubrik |
| `Description` | Kort beskrivning |
| `Thumbnail` | Förhandsvisningsbild (url) |
| `File URL` | Länk till fil (url) |
| `Button Text` | CTA-text (default: "Ladda ner PDF") |
| `Order` | Sortering |
| `Visa` | Visa/dölj |

Korten har flex-column med description `flex:1` → knappar alignas i botten.

**`compare`** — Jämförelsetabell

| Fält | Beskrivning |
|------|-------------|
| `Compare Title` | Rubrik ovanför tabell |
| `Compare Col A` | Kolumnrubrik A |
| `Compare Col B` | Kolumnrubrik B (grön highlight) |
| `Compare Rows` | En rad per tabellrad. Format: `Label \| Värde A \| Värde B` |

Vit kort på mörkblå bakgrund. Kolumn B har grön highlight. Zebra striping.

**`steps`** — Numrerade steg

| Fält | Beskrivning |
|------|-------------|
| `Steps Title` | Rubrik |
| `Steps` | En rad per steg. Format: `Stegrubrik \| Beskrivning` |

Cirkel-numrering (solid #1E3F6E) med kopplande linje. Hover: orange.

#### 4. Kontaktperson

Horisontellt kort (desktop) med foto, kontaktinfo och citat. Mobil: mörkt kort med citattecken.

| Fält | Typ | Beskrivning |
|------|-----|-------------|
| `Contact Name` | singleLineText | Namn |
| `Contact Title` | singleLineText | Titel/roll |
| `Contact Email` | email | E-post |
| `Contact Phone` | phoneNumber | Telefon |
| `Contact Image` | url | Profilbild-URL |
| `Contact Quote` | multilineText | Citat |
| `Show Contact` | checkbox | Visa/dölj |

### Färg-overrides

| CSS-variabel | Default | Airtable-fält |
|--------------|---------|---------------|
| `--lp-main` | `#11325D` | `Color Main` |
| `--lp-secondary` | `#F28C28` | `Color Secondary` |

**Designprincip:** Orange = action (CTA-knappar, submit). Mörkblått = information, trovärdighet.

### Formformatregler

Dessa fält kräver specifikt format. Fel format syns först i WordPress.

| Fält | Format | Exempel |
|------|--------|---------|
| Content Benefits | En per rad | `Benefit 1\nBenefit 2\nBenefit 3` |
| Case Outcomes | En per rad | `Utfall 1\nUtfall 2` |
| FAQ Items | `Q: \nA: \n\n` (tom rad mellan) | `Q: Fråga\nA: Svar\n\nQ: Fråga 2\nA: Svar 2` |
| Compare Rows | `Label \| A \| B` per rad | `Hastighet \| 1 Gbit \| 10 Gbit` |
| Steps | `Rubrik \| Beskrivning` per rad | `Analys \| Vi kartlägger systemet` |

### Enfold CSS-isolation

Unikt ID per instans: `#wexoe-lp-{uniqid}`. All CSS scopas med `!important`.

| Enfold-problem | Fix i pluginet |
|----------------|----------------|
| `min-width: 270px` på knappar | `min-width: 0 !important` |
| `background-image` (pil) på knappar | `background-image: none !important` |
| `background: #f8f8f8` på tabeller | `background: transparent !important` på table/tr/td |
| List-ikon (checkmark) | Killed via `.avia-builder` override |
| `float` på knappar | `float: none !important` |

### Versionshistorik

| Version | Ändringar |
|---------|-----------|
| 1.0.0 | Initial release — hero, content, 4 sidebar-typer, 7 tab-typer, kontaktperson, full Enfold isolation |
...
|---------|-------|-----------|
| 1.0.0 | 2026-04-08 | Initial release — hero med diagonal bildklippning, content + 4 sidebar-typer (case/calculator/event/leadmagnet), tabs med 7 content-typer (textimage/fullmedia/faq/calameo/downloads/compare/steps), kontaktperson, full Enfold CSS-isolation |
